...
Analyze this event and give sharp, actionable buying advice:
...
- Explain what the price range tells us about demand for this event
- Give a clear "best time to buy" recommendation
- Highlight anything notable about this event's market
- Sound like a smart friend who knows ticketing, not a robot

Be direct, specific, and genuinely useful."""
...
category = st.sidebar.selectbox(
    "Category",
    ["All", "Music", "Sports", "Arts & Theatre", "Comedy"]
)
...
st.sidebar.markdown(
    ...
    "AI analysis powered by Claude.</p>",
    unsafe_allow_html=True
)
...
st.markdown(
    ...
    "Search any event. Understand the market. Know when to buy.</p>",
    unsafe_allow_html=True
)
st.markdown("---")
...
with col1:
    query = st.text_input(
        "",
        placeholder="Search for an artist, team, show, or venue...",
...
if st.session_state.events:
    st.markdown(f"## {len(st.session_state.events)} Events Found")
...
if st.session_state.selected_event:
    ...
    st.markdown(
        f"<p style='color:#888;'>📍 {venue.get('name', '')}, "
        f"{venue.get('city', {}).get('name', '')}, "
        f"{venue.get('state', {}).get('name', '')} "
        f"&nbsp;•&nbsp; 📅 {date_fmt}</p>",
        unsafe_allow_html=True
    )
    ...
    if price_ranges:
        ...
        if min_price and max_price:
            ...
            ax.barh(["Ticket Prices"], [max_price - min_price],
                    left=[min_price], color="#333333", height=0.4)
            mid = (min_price + max_price) / 2
            ax.barh(["Ticket Prices"], [max_price * 0.05],
                    left=[mid], color="#f5c842", height=0.4)

            ax.text(min_price, 0, f" ${min_price:.0f}",
                    va="center", fontsize=9, color="#888")
            ax.text(max_price, 0, f"${max_price:.0f} ",
        ...
        st.info("Pricing information not available for this event yet.")
    ...
    if classifications:
        ...
        if segment or genre:
            st.markdown(
                ...
                f"Category: {segment} {'— ' + genre if genre else ''}</p>",
                unsafe_allow_html=True
            )
    ...
    if ticket_url:
        st.markdown(
            ...
            f"<a href='{ticket_url}' target='_blank' style='color:#f5c842;'>"
            f"→ Buy tickets on Ticketmaster</a></p>",
            unsafe_allow_html=True
        )
    ...
    if st.session_state.ai_insight:
        st.markdown(
            f'<div class="ai-box">'
            f'<div class="ai-label">✦ AI Market Insight</div>'
            f'<div class="ai-text">{st.session_state.ai_insight}</div>'
            f'</div>',
            unsafe_allow_html=True
        )

elif not st.session_state.events and not search_clicked:
    st.markdown(
        "<p style='color:#555; text-align:center; margin-top:3rem; font-size:1.1rem;'>"
        "Search for any artist, team, show, or venue to get started.</p>",
        unsafe_allow_html=True
    )
